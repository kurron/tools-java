...
curl --include --request PUT --user "kurron:${KEY}" --header "X-Bintray-Package:java-tools" --header "X-Bintray-Version:${RELEASE}" --header "Content-Type: application/octet-stream" --header "Accept:application/json" --data-binary @build/libs/tools-java-0.0.0-SNAPSHOT.jar https://api.bintray.com/maven/kurron/maven/java-tools/org.kurron.example/tools-java/${RELEASE}/tools-java-${RELEASE}.jar

curl --verbose --request POST --user "kurron:${KEY}" --header "Content-Type: application/json" https://api.bintray.com/content/kurron/maven/java-tools/${RELEASE}/publish 
